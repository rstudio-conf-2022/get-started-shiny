...
shinyApp(
    ...
    dashboardBody( 
      fluidRow(
        box(
          ...
          solidHeader = TRUE, status = "primary",          plotOutput("plot")
        )
      ),
      fluidRow(
  ...
  server = function(input, output, session) {
    ...
    output$min_temp = renderInfoBox({
      ...
      min_temp_time = d_city()$time[d_city()$temperature == min_temp][1]
    ...
    output$max_temp = renderInfoBox({
      ...
      max_temp_time = d_city()$time[d_city()$temperature == max_temp][1]
