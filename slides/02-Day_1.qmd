...
The modal dialog consists of a number of Shiny UI elements (static or dynamic) and only displays when it is triggered (usually by something like an action button).

They differ from other UI elements we've seen so far as everything is defined within the app's server component.
